...
/*  like controllers, here are all the like controllers we use SQL DB   ** 
**  "like" to make users able to add a like to a post                   **
**  in the same way, user can anadd his like and it will be remove      **
**  from the DataBase                                                   **
**  "liked" to get back  user's like                                    */
                 
////////////////////////////  add and anadd Like Function  ////////////////////////////
...
exports.like = (req, res, next) => {
    ...
    pool.execute(sql, function (err, results) {
        ...
        if (!userLiked) {
            ...
            pool.execute(sql,[req.body.postId], function (err, result) {
                if (err) res.status(400).json({ err });
                let sql2 = `DELETE FROM groupomania.like WHERE like.postId = ? AND like.userId = ?`;
                pool.execute(sql2,[req.body.postId,req.body.userId], function (err, result) {
                    if (err) res.status(400).json({ err });
                    res.status(200).json(result)
                });
